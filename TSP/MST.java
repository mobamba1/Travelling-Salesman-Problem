...
public class MST
{
	...
	public static double[][] PrimsMST(double[][] d)
	{
		...
		Tour = (ArrayList<Integer>) v.clone();
		return(res);
	}
	
	public static double Mfitness(double[][] distance){
		double [][] Mdistance = PrimsMST(distance);				
		double fitness = 0;
		...
		for(int i = 0; i < a; i++){
			for(int j = 0; j < a; j++){
				fitness += Mdistance[i][j];
				
			}
		}
		int start = Tour.get(0);
		int end = Tour.get(b -1);
